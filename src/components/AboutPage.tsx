  ...
  return (
    <div style={{ background: colors.bgCream, minHeight: "100vh" }} className="pt-20">
                ...
                {collageImages.map((img, idx) => (
                  ...
                  >
                    <motion.img
                      ...
                      loading="lazy"
                      initial={{ scale: 1 }}
                      whileHover={{ scale: 1.15, transition: { duration: 0.5, ease: [0.4, 0, 0.2, 1] } }}
                    />
                    ...
                    <div
                      className="absolute inset-0 pointer-events-none opacity-10 hover:opacity-5 transition-opacity duration-300"
                    ...
                    <motion.div
                      ...
                      whileHover={{ scale: 1.5, borderColor: colors.accentGold, transition: { duration: 0.3 } }}
                    />
                    <motion.div
      ...
      <section ref={storyRef} className="py-28 relative overflow-hidden" style={{ background: colors.bgLight }}>
        <div className="absolute inset-0">
          <div className="absolute top-20 left-10 w-32 h-32 opacity-5" style={{ background: colors.accentGold, borderRadius: "50%", filter: "blur(40px)", animation: "pulse 6s infinite" }} />
          <div className="absolute bottom-20 right-10 w-48 h-48 opacity-5" style={{ background: colors.accentOrange, borderRadius: "50%", filter: "blur(48px)", animation: "pulse 6s infinite 3s" }} />
        </div>
        <div className="container mx-auto px-6 lg:px-8 relative z-10">
          <motion.div initial="hidden" animate={isStoryInView ? "visible" : "hidden"} className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            {/* Story Content */}
            <motion.div
            ...
            >
              ...
              <div className="space-y-7 font-netflix text-lg" style={{ color: colors.textDark, opacity: 0.85, lineHeight: "1.8" }}>
                <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.7 }} viewport={{ once: true }}>
                  Founded in 2018, Moneo Films is a trailblazing production company owned entirely by Black women. We are driven by purpose, passion, and creative excellence.
                </motion.p>
                <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.7 }} viewport={{ once: true }}>
                  Led by visionary co-founders Ferry Jele and Neo R Paulus, we push boundaries and redefine storytelling across Africa and beyond.
                </motion.p>
                <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.7 }} viewport={{ once: true }}>
                  We produce high-impact films, documentaries, series, and a range of television content. But we are more than just a production company — we are a fully integrated entertainment and administration hub.
                </motion.p>
              </div>
              {/* Stats Cards */}
              <motion.div
              ...
              >
                ...
                >
                  <div className="font-light text-4xl mb-3 font-netflix" style={{ color: colors.accentOrange }}>
                    2018
                  </div>
                  <div className="font-light text-sm uppercase tracking-[0.2em] font-netflix" style={{ color: colors.textDark, opacity: 0.7 }}>
                    Founded
                  </div>
                </motion.div>
                <motion.div
                  ...
                  whileHover={{ scale: 1.05 }}
                >
                  <div className="font-light text-4xl mb-3 font-netflix" style={{ color: colors.accentGold }}>
                    100%
                  </div>
                  <p className="font-light text-sm uppercase tracking-[0.2em] font-netflix" style={{ color: colors.textDark, opacity: 0.7 }}>
                    Black Women Owned
                  </p>
                </motion.div>
              </motion.div>
            ...
            >
              ...
              >
                <div className="relative w-full h-full">
                  {carouselImages.map((image, index) => (
                    <motion.div key={image.id} className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${index === currentSlide ? "opacity-100" : "opacity-0"}`}>
                      <img src={image.src || "/placeholder.svg"} alt={image.alt} className="w-full h-full object-cover" loading="lazy" />
              ...
              {/* Decorative Element */}
              <div className="absolute -z-10 -right-6 -bottom-6 w-32 h-32 rounded-full" style={{ background: colors.accentGold, opacity: 0.1, filter: "blur(20px)" }} />
            </motion.div>
          </motion.div>
          {/* Open Story Button */}
          <motion.div
...
<AnimatePresence>
  {showStoryModal && (
    ...
    >
      <motion.div
        className="bg-white rounded-lg p-8 shadow-xl relative w-full max-w-2xl mx-4"
        initial={{ scale: 0.95 }}
        animate={{ scale: 1 }}
        exit={{ scale: 0.95 }}
        onClick={(e) => e.stopPropagation()}
      ...
      >
        <button
          className="absolute top-4 right-4 rounded-full p-2 transition-colors z-10 bg-[#CC5500] text-white hover:bg-[#CC5500]/90"
          onClick={() => setShowStoryModal(false)}
          aria-label="Close Story Modal"
        >
          <X className="w-5 h-5" />
        </button>
        <div className="pr-4"> {/* Add padding to prevent content from being hidden behind scrollbar */}
          ...
          <div className="space-y-6 font-netflix text-lg" style={{ color: colors.textDark, opacity: 0.8, lineHeight: "1.7" }}>
            <p>Founded in 2018, Moneo Films is a trailblazing production company owned entirely by Black women. We are driven by purpose, passion, and creative excellence.</p>
            <p>Led by visionary co-founders Ferry Jele and Neo R Paulus, we push boundaries and redefine storytelling across Africa and beyond.</p>
            <p>We produce high-impact films, documentaries, series, and a range of television content. But we are more than just a production company — we are a fully integrated entertainment and administration hub.</p>
            <p>Our team brings expertise across every stage of production, including producing, technical producing, script development, directing, cinematography, Photography, editing, performance and dialogue coaching, technical support, and web & app development.</p>
            <p>We also invest in the future through free training and mentorship, empowering aspiring creatives to succeed in the industry — not for recognition, but to build the next generation of storytellers.</p>
            <p>At Moneo Films, we're not just building productions — we're building a legacy grounded in excellence, collaboration, and transformation through storytelling.</p>
            <p>We pride ourselves on delivering excellence in every frame, every performance, and every experience. Storytelling isn't just what we do — it's who we are.</p>
          </div>
        </div>
      </motion.div>
    </motion.div>
      ...
      <section className="py-20 relative overflow-hidden" style={{ background: colors.bgLight }}>
        <div className="container mx-auto px-6 lg:px-8 relative z-10">
              ...
              >
                ...
                >
                  ...
                  <div className="relative aspect-[4/5] overflow-hidden">
                    ...
                    >
                      <motion.span className="text-white font-medium text-sm opacity-0" whileHover={{ opacity: 1, transition: { delay: 0.1 } }}>
                        View Profile
                      </motion.span>
                    </motion.div>
                  </div>
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <div className="mb-4">
                      ...
                      <div
                        ...
                        style={{ background: `${colors.accentOrange}15`, color: colors.accentOrange, border: `1px solid ${colors.accentOrange}30` }}
                      >
                        {member.role}
                      </div>
                    </div>
                    <p className="font-light text-sm leading-relaxed minimal-text font-netflix flex-1" style={{ color: colors.textDark, opacity: 0.8, lineHeight: "1.6" }}>
                      {member.bio}
                    </p>
                    {/* Social Indicators */}
                    <motion.div className="flex justify-center gap-4 mt-6 pt-4 border-t border-[#2A1A12]10" initial={{ opacity: 0 }} whileInView={{ opacity: 1, transition: { delay: 0.4 } }}>
          ...
          {/* Decorative Elements */}
          <div className="absolute top-1/4 left-10 w-24 h-24 opacity-10" style={{ background: colors.accentGold, borderRadius: "50%", filter: "blur(20px)", animation: "pulse 4s infinite" }} />
          <div className="absolute bottom-1/4 right-10 w-20 h-20 opacity-10" style={{ background: colors.accentOrange, borderRadius: "50%", filter: "blur(15px)", animation: "pulse 4s infinite 2s" }} />
        </div>
      </section>
